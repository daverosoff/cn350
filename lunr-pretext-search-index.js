...
},
{
  ...
  "body": " Intro to pushdown automata    Learn stuff     Informal definition  We imagine a pushdown automaton or PDA as being like an -NFA with a stack attached. We still use transitions to move from one state to another. Like -NFA, PDA are nondeterministic and can spontaneously transition without consuming input. This is not a big new idea.  On the other hand, the transition function for a PDA uses both the input word and the stack as part of the input. That is, the transitions that are available are determined in part by the contents of the stack as well as the remaining part of the input word.  Both -transitions and normal transitions have the chance to alter the stack on their way to the new state. A transition may:  Pop the top symbol off the stack (it vanishes forever)  Push one or more symbols onto the top of the stack  Both these alterations are optional, but if both occur, the pop is always first.  In the formal specification given in , you will see that the set of stack symbols is allowed to be different from the alphabet . In fact, it always is different, because we need a special start symbol that has no job but being the bottom of the stack.  Acceptance for PDA is somewhat like acceptance for NFA. PDA have a set of states designated as accepting states, and if there is a branch of the nondeterministic computation that lands in an accepting state when the input word is consumed, the word is said to be in the accepting-state language of the PDA. As the phrasing suggests, there is a second definition of acceptance that is postponed until HMU section 6.2. This is called empty-stack acceptance, and we will see it is quite natural.   For any particular PDA , the accepting-state language and the empty-stack language are typically different.      Formal specification  The formal specification of a PDA looks like this: . Here   is the set of states   is the alphabet (the input symbols)   is the set of stack symbols   is the transition function, explained in more detail below   is the initial state (we assume )   is the bottom stack symbol (we assume that )   is the set of accepting states (we assume that )  We usually don't give tables for the transition function, instead preferring lengthy verbal descriptions. The reason is that the domain is the product of three sets, namely and thus instead of a 2-d table of input values we have a 3-d cube of them.  Here are some useful figures. I'll add more explanation later.   (for accessibility)    (for accessibility)    (for accessibility)     (This is Exercise 6.1.1 from HMU.) Suppose that is a PDA, and that is described by the following rules.    . This means that if in state , we read a and there is on the top of the stack, we can (and must) remain in state , consume the , do not pop, and push an on the stack. The pair represents the new state and the new stack top (read from top to bottom as we go left to right, so the is still there and the is on top of it).                                   Show all the reachable instantaneous descriptions when the input is:           Converting PDA to CFG   In this section we outline the constructions of a CFG from a PDA and vice versa. The constructions are abstract, and we only give sketches of the proofs, which are necessarily also quite abstract. They are the most difficult we will encounter in the course. However, the constructions are easily understood through examples, and they make it intuitively clear that the generality is complete. That is to say, there is no restriction on which PDA and CFG may be so converted.    CFG to PDA  Let the grammar be specified as , where is the set of productions since we will use for the PDA that is equivalent to . Recall that each production in has the form , where and , that is, is a string of stack symbols.  The formal specification of the empty-stack PDA is . This says:    has only one state,     The terminals of are the input alphabet of     The terminals and variables of are the stack alphabet of     The transition function is described below    The start state of is     The start symbol of is , the start symbol fo ( not  !)    We don't specify any final states, since they are irrelevant (this machine will accept by empty stack)     We now describe the transitions of . This isn't as bad as you think!   For each variable of , we have     For each terminal of , we have      That's it! To see how it works, let's look at the example from HMU Example 6.12.    Evidently the input alphabet is and the stack alphabet is those symbols together with and .  Observe that the string has a leftmost derivation . Let us use the PDA to recognize this string. The starting ID is . We use the transition suggested by the leftmost derivation, . The resulting ID is , because we consume and replace at the top of the stack with .  Next, we still have on the top of the stack, which is good, because the next step in our leftmost derivation is , which uses the transition . Applying this transition, we see that the next ID is .  The next step in the derivation is , coming from the transition . Applying it, we find the next ID is . Now something interesting can happen. We see that the only available transition is specified by . This means we consume the from the left end of the input and pop the matching off the stack, replacing it with nothing. The next ID is thus .   The mechanism should now be clear, as should the paragraphs of HMU that precede example 6.12 about how the IDs of the computation on the PDA side simulate left sentential forms on the CFG side.  The leftmost derivation is thus exactly simulated. At each step, we pop off any matching string of terminals at the left side of the remaining input. Then we apply the transition suggested by the next step of the derivation. This will remove the leftmost variable and replace it by the right-hand side of the transition. We repeat until we have popped the entire input off the stack (since it will have been placed there one piece at a time by our transitions) and then can accept the word.    Finish the chain of IDs for that begins . Since we began with a legal expression (it is generated by a -leftmost derivation), your chain of IDs should end with .      PDA to CFG  Here we begin with an empty-stack PDA . Since must empty its stack to accept, we can simplify our thinking about the chain of IDs it goes through by only thinking about when each symbol is popped. As HMU say, if a sequence is on the stack, may undergo many transitions before is eventually popped (as it must be, since accepts by empty stack). Let us denote by the part of the input that is consumed during these transitions. It is irrelevant what other stack symbols may be pushed and popped by all these transitions, because by the time is fully consumed, is popped, which means that everything above it was also popped by consumption of .  In the same manner, let us denote by and the state in which finds itself at the beginning and end of , respectively. Thus we have a chain of IDs that (shortened) looks like this: . Chains like this form the fundamental events of our computation and the form our grammar will take.  Indeed, if has the formal specification (again we may omit the final states), then let be specified by , where the variables consists of  The special grammar symbol , the start symbol  All symbols of the form , where , , and .    Recall now that is the set of targets of available transitions with consumed and on the stack in state . Such a target is a pair where is the new state and replaces on the top of the stack.  The productions of are as follows:  For each state , a production  A production for every tuple , whenever contains .  It will then follow (not at all obviously) that . This is not easy to understand without an example.   As in HMU example 6.15, let's convert the PDA that detects invalid if-else chains to a CFG. The PDA is the empty-stack machine given by , where is given by . This machine rejects as soon as the first impossible else is detected.  Then according to the description above, we have two variables in , namely and (since there is only one state and one stack symbol).  The only -production is , as required by item 1 above. If there were more states, we would have more productions of this type.  Because contains , we need a production of the form 2 above with , , and . That is, . If there were states, this step would produce productions, since the middle two s above could be replaced by any other state.  Finally, because contains , we have a production . The lack of variables here is because this is coming from a transition that pops the stack.  The grammar then looks like this: . As HMU point out, it hurts nothing to relabel the variables, so we can state this grammar also as . One can even notice that the variable is not really needed and compress it further to .     Now consider the PDA that accepts valid if-else chains, with two states and , and . Here is the start symbol. Convert this to an equivalent context-free grammar.      "
},
{
  "id": "objectives-6",
  "level": "2",
...
{
  "id": "example-cfg-to-pda",
  "level": "2",
  "url": "PDA-intro.html#example-cfg-to-pda",
  "type": "Example",
  "number": "3.2.2",
  "title": "",
  "body": "  Evidently the input alphabet is and the stack alphabet is those symbols together with and .  Observe that the string has a leftmost derivation . Let us use the PDA to recognize this string. The starting ID is . We use the transition suggested by the leftmost derivation, . The resulting ID is , because we consume and replace at the top of the stack with .  Next, we still have on the top of the stack, which is good, because the next step in our leftmost derivation is , which uses the transition . Applying this transition, we see that the next ID is .  The next step in the derivation is , coming from the transition . Applying it, we find the next ID is . Now something interesting can happen. We see that the only available transition is specified by . This means we consume the from the left end of the input and pop the matching off the stack, replacing it with nothing. The next ID is thus .  "
},
{
  "id": "activity-43",
  "level": "2",
  "url": "PDA-intro.html#activity-43",
  "type": "Activity",
  "number": "3.2.2",
  "title": "",
  "body": "  Finish the chain of IDs for that begins . Since we began with a legal expression (it is generated by a -leftmost derivation), your chain of IDs should end with .   "
},
{
  "id": "example-7",
  "level": "2",
  "url": "PDA-intro.html#example-7",
  "type": "Example",
  "number": "3.2.3",
  "title": "",
  "body": " As in HMU example 6.15, let's convert the PDA that detects invalid if-else chains to a CFG. The PDA is the empty-stack machine given by , where is given by . This machine rejects as soon as the first impossible else is detected.  Then according to the description above, we have two variables in , namely and (since there is only one state and one stack symbol).  The only -production is , as required by item 1 above. If there were more states, we would have more productions of this type.  Because contains , we need a production of the form 2 above with , , and . That is, . If there were states, this step would produce productions, since the middle two s above could be replaced by any other state.  Finally, because contains , we have a production . The lack of variables here is because this is coming from a transition that pops the stack.  The grammar then looks like this: . As HMU point out, it hurts nothing to relabel the variables, so we can state this grammar also as . One can even notice that the variable is not really needed and compress it further to .  "
},
{
  "id": "activity-44",
  "level": "2",
  "url": "PDA-intro.html#activity-44",
  "type": "Activity",
  "number": "3.2.3",
  "title": "",
  "body": "  Now consider the PDA that accepts valid if-else chains, with two states and , and . Here is the start symbol. Convert this to an equivalent context-free grammar.   "
},
{
  "id": "sec-hw1",
  "level": "1",
